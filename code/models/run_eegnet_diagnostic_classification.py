...
def main() -> int:
    ...
    x_full = np.load(args.data_dir / "X_first_fixation_len256.npy", mmap_mode="r")
    mask_full = np.load(args.data_dir / "mask_first_fixation_len256.npy", mmap_mode="r")
    x = x_full
    mask = mask_full
    ...
    for fold_id, (subject, outer_train_idx, test_idx) in enumerate(loso_folds(subjects)):
        inner_train_idx, val_idx = stratified_train_val_split(outer_train_idx, y, args.val_size, args.seed + fold_id)
        print(
    ...
    stem = (
        f"zuco2_nr_diagnostic_first_fixation_len256_loso_eegnet_{args.target}{tag_part}"
