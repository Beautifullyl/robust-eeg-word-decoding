...
The [reproduction guide](documentation/REPRODUCTION_GUIDE.txt) explains how to set
up the environment, locate the data and run the experiments on CSF3. The
[technical appendix](documentation/TECHNICAL_APPENDIX.txt) summarises the methods.
...
3. Extract the 512-sample all-fixations representation (Workflow 03).
4. Run the main all-fixations EEGNet LOSO baseline (Workflow 04).
5. Run the TCN model comparison (Workflow 05).
6. Run the linear baseline, diagnostic splits and per-class analysis
   (Workflow 06).
7. Run structured channel-loss experiments (Workflow 07).
8. Audit subject and sentence overlap between splits (Workflow 08).
9. Run the non-EEG control models (Workflow 09).
10. Run LOSO with grouped inner validation (Workflow 10).
11. Repeat the sentence-disjoint evaluation across five sentence partitions
    (Workflow 11).
12. Run persistent random channel loss (Workflow 12).
...
The workflow numbers match the script filenames and run consecutively.
...
```bash
python code/run_pipeline.py submit workflow12 --mode formal
```
...
Workflow 14 waits for the required experiments and analyses to finish, then
...
included here. See [data availability](documentation/DATA_AVAILABILITY.txt) for
access details and the [reproduction guide](documentation/REPRODUCTION_GUIDE.txt)
